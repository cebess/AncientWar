...
import com.cebess.ancientwar.screens.GameFieldScreen;
...
public class Field {
    ...
    public static enum SideAction {advance,retreat,standstill,normal}
    ...
    public BattleElement getFieldValue(int j, int i) throws IllegalArgumentException {
        if (i<0 || i>= MAX_HEIGHT)
            throw new IllegalArgumentException("X location invalid: " + i);
        if (j<0 || j>= MAX_WIDTH)
            throw new IllegalArgumentException("y location invalid: " + j);
        for (BattleElement testElement:field) {
            // check for a value at the location
            if (testElement.getLoc_X() == i  && testElement.getLoc_Y() == j) {
                return (testElement);
            }
        }
        return(null);
    ...
    public void add(BattleElement newElement) throws IllegalArgumentException {
        ...
        else throw new IllegalArgumentException("BattleElement already at this location: " + test_xloc + "," +test_yloc);
    }
